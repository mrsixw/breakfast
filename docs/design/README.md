# Design Documents

Technical design documents for planned or in-progress features.

Each document captures the problem, proposed solution, and implementation approach for a feature before development begins. Once a feature is implemented, the design doc remains as a historical record of the decision-making process.

## Documents

| Document | Status | Issue |
| --- | --- | --- |
| [Config File](config-file.md) | Proposed | [#42](https://github.com/mrsixw/breakfast/issues/42) |
| [Demo Generation](demo-generation.md) | Implemented | [#80](https://github.com/mrsixw/breakfast/issues/80) |
| [PR Caching](pr-caching.md) | Proposed | [#45](https://github.com/mrsixw/breakfast/issues/45) |
| [Text User Interface](tui.md) | Proposed | [#46](https://github.com/mrsixw/breakfast/issues/46) |
| [Pluggable Seasonal Calendars](pluggable-calendars.md) | Implemented | [#196](https://github.com/mrsixw/breakfast/issues/196) |
| [Testing Strategy](testing.md) | Implemented | [#99](https://github.com/mrsixw/breakfast/issues/99) |
