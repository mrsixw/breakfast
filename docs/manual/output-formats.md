...
|       2 | platform-ui    | WIP: nav redesign | dave   | open (draft)   |   8   |    2    |  +120/-30  |    0     | ✅ (clean)   | PR-91  |
|       3 | platform-ui    | Update nav bar    | carol  | open           |  12   |    4    |  +280/-95  |    1     | ❌ (dirty)   | PR-87  |
+---------+----------------+-------------------+--------+----------------+-------+---------+------------+----------+--------------+--------+
```
...
| State | PR status: `open` (green), `open (draft)` (yellow) for draft PRs, `closed` (red) |
...
The **State** column is colour-coded by status:
- **Green** (`open`) — ready for review
- **Yellow** (`open (draft)`) — draft PR, not yet ready for review
- **Red** (`closed`) — closed PR
